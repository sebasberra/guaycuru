...
SELECT
	...
	AND s1.nombre = (
		SELECT
			s0.nombre
		FROM
			alassia_hmi2_20161111.salas s0
		WHERE
...
FROM
	...
	alassia_hmi2_20161111.habitaciones h0;
	...
	WHERE
		h1.nombre = (
			SELECT
				h0.nombre
			FROM
				alassia_hmi2_20161111.camas c0
			INNER JOIN
				alassia_hmi2_20161111.habitaciones h0
			ON
	...
	hmi2guaycuru_camas.servicios_get_id_servicio_sala(
		...
		hmi2guaycuru_camas.servicios_get_id_servicio_estadistica(
			...
			(SELECT
				es0.cod_servicio
			FROM
				alassia_hmi2_20161111.efectores_servicios es0
			WHERE
...
FROM
	...
	alassia_hmi2_20161111.servicios_salas ss
