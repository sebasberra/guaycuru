...
FROM
	...
	reconquista_hmi2_20170208.salas s
...
SELECT
	...
	AND s1.nombre = (
		SELECT
			s0.nombre
		FROM
			reconquista_hmi2_20170208.salas s0
		WHERE
...
FROM
	...
	reconquista_hmi2_20170208.habitaciones h0;
	...
	WHERE
		h1.nombre = (
			SELECT
				h0.nombre
			FROM
				reconquista_hmi2_20170208.camas c0
			INNER JOIN
				reconquista_hmi2_20170208.habitaciones h0
			ON
...
FROM
	...
	reconquista_hmi2_20170208.camas c;
	...
	hmi2guaycuru_camas.servicios_get_id_servicio_sala(
		...
		hmi2guaycuru_camas.servicios_get_id_servicio_estadistica(
			...
			(SELECT
				es0.cod_servicio
			FROM
				reconquista_hmi2_20170208.efectores_servicios es0
			WHERE
...
FROM
	...
	reconquista_hmi2_20170208.servicios_salas ss
