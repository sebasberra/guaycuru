...
SELECT
	...
	AND s1.nombre = (
		SELECT
			s0.nombre
		FROM
			coronda_hmi2_20160920.salas s0
		WHERE
...
FROM
	...
	coronda_hmi2_20160920.habitaciones h0;
	...
	WHERE
		h1.nombre = (
			SELECT
				h0.nombre
			FROM
				coronda_hmi2_20160920.camas c0
			INNER JOIN
				coronda_hmi2_20160920.habitaciones h0
			ON
	...
	hmi2guaycuru_camas.servicios_get_id_servicio_sala(
		...
		hmi2guaycuru_camas.servicios_get_id_servicio_estadistica(
			...
			(SELECT
				es0.cod_servicio
			FROM
				coronda_hmi2_20160920.efectores_servicios es0
			WHERE
...
FROM
	...
	coronda_hmi2_20160920.servicios_salas ss
